...
export default function SlotListTable() {
  //Router Hook
  ...
  //Detail Modal Hook
  const {isModalOpen,selectedItem,fetchDetail,closeModal: closeDetailModal} = useDetailModal<ISlot>('/slots')

  //Edit Modal Hook
  const {isOpen: isEditModalOpen,itemToEdit: slotToEdit,openEdit: handleEditClick,closeEdit: closeEditModal} = useEditModal<ISlot>()
  ...
  return (
    ...
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.5 }}>
      ...
      <TableHeaderActions
        ...
        onAddNew={() => router.push('/admin/slots/add')}
        onPrint={() => window.print()}
        onExport={() => exportExcel({ data, fileName: 'Slots', sheetName: 'Slots' })}
      ...
      />
        ...
        {/* Detail Modal */}
      <Modal isOpen={isModalOpen} onClose={closeDetailModal} title="Slot Details">
        {selectedItem && (
          <SlotDetail slot={selectedItem} />
        )}
      </Modal>
